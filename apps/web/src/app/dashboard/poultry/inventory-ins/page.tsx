...
interface Row { id: string; doc_no: string; transaction_date: string; status: string; supplier_name: string; supplier_code: string; remarks: string | null; }
...
export default function InventoryInsPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api.get<{ data: Row[]; total: number }>(`/poultry/inventory-ins?company_id=${cid}&limit=${PAGE}&offset=${(page - 1) * PAGE}${status ? `&status=${status}` : ''}`)
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <div><h1 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Inventory In</h1><p className="text-sm text-slate-500">Chick receiving transactions.</p></div>
        <Link href="/dashboard/poultry/inventory-ins/new" className="rounded bg-brand-600 px-4 py-2 text-sm font-medium text-white hover:bg-brand-700">+ New Inventory In</Link>
      </div>
      ...
      <div className="overflow-hidden rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {loading ? <tr><td colSpan={5} className="py-8 text-center text-slate-400 text-xs">Loading…</td></tr>
              : !rows.length ? <tr><td colSpan={5} className="py-8 text-center text-slate-400 text-xs">No records found.</td></tr>
              : rows.map(r => (
                <tr key={r.id} className="border-b border-slate-50 dark:border-slate-700 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800">
                  <td className="px-3 py-2"><Link href={`/dashboard/poultry/inventory-ins/${r.id}`} className="font-mono text-brand-600 hover:underline">{r.doc_no}</Link></td>
