...
interface Row { id: string; doc_no: string; transaction_date: string; date_needed: string | null; status: string; total_amount: number; supplier_name: string; supplier_code: string; remarks: string | null; purchase_order_id: string | null; po_no: string | null; }
      ...
      <div className="overflow-hidden rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">
        <table className="min-w-full text-sm">
          <thead className="border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-xs text-slate-500">
            <tr>
              <th className="px-3 py-2 text-left">Order In</th>
          ...
          <tbody>
            {loading ? <tr><td colSpan={7} className="py-8 text-center text-slate-400 text-xs">Loading…</td></tr>
              : !rows.length ? <tr><td colSpan={7} className="py-8 text-center text-slate-400 text-xs">No records found.</td></tr>
              : rows.map(r => (
                <tr key={r.id} className="border-b border-slate-50 dark:border-slate-700 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800">
                  ...
                  <td className="px-3 py-2">
                    {r.po_no && r.purchase_order_id
                      ? <Link href={`/dashboard/purchasing/purchase-orders/${r.purchase_order_id}`} className="font-mono text-xs text-brand-700 hover:underline dark:text-brand-400">{r.po_no}</Link>
