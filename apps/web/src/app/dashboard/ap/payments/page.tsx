...
export default function PaymentsPage() {
  ...
  useEffect(() => {
    ...
    const q = status === 'all'
      ...
      : `/ap/payments?company_id=${companyId}&status=${status}&limit=500`;
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Supplier Payments</h1>
          <p className="text-sm text-slate-600 dark:text-slate-400">Check vouchers and bank payment records.</p>
        </div>
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={7} className="px-3 py-8 text-center text-xs text-slate-500">No payments found.</td></tr>
            ) : paged.map((r) => (
              <tr key={r.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-800">
                <td className="px-3 py-2">
                  <Link href={`/dashboard/ap/payments/${r.id}`} className="font-mono text-xs text-brand-700 hover:underline dark:text-brand-400">
                    {r.voucher_no}
                  </Link>
                </td>
                <td className="px-3 py-2 text-xs text-slate-600 dark:text-slate-400">{formatDate(r.payment_date)}</td>
                <td className="px-3 py-2">
                ...
                <td className="px-3 py-2 text-xs capitalize text-slate-600 dark:text-slate-400">{r.payment_method.replace(/_/g, ' ')}</td>
                <td className="px-3 py-2 text-xs text-slate-600 dark:text-slate-400">{r.reference ?? '—'}</td>
                <td className="px-3 py-2 text-right font-mono text-xs font-semibold dark:text-slate-300">{formatPHP(r.amount)}</td>
                <td className="px-3 py-2">
