...
function NewBillForm() {
  ...
  const [form, setForm] = useState({ supplier_id: '', bill_no: '', bill_date: new Date().toISOString().split('T')[0], due_date: '', po_id: params.get('po_id') ?? '' });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const bill = await api.post<{ id: string }>('/ap/bills', {
        ...
        due_date: form.due_date || undefined, po_id: form.po_id || undefined,
          ...
          <table className="min-w-full text-xs">
            <thead className="border-b border-slate-200 bg-slate-50 text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400">
              <tr>
                ...
                <th className="px-2 py-1.5 text-right font-medium w-14">EWT %</th>
            ...
            <tbody>
              {lines.map((l, idx) => (
                <tr key={idx} className="border-b border-slate-100 dark:border-slate-700">
                  ...
                  <td className="px-2 py-1"><input type="number" min={0} step="any" value={l.ewt_rate} onChange={e => updateLine(idx, 'ewt_rate', parseFloat(e.target.value) || 0)} className="w-full rounded border border-slate-300 px-1 py-1 text-right dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100" /></td>
            ...
            <tfoot>
              <tr className="bg-slate-50 dark:bg-slate-800"><td colSpan={8} className="px-2 py-1.5 text-right text-xs text-slate-500 dark:text-slate-400">Grand Total (incl. VAT)</td><td className="px-2 py-1.5 text-right font-mono text-xs text-slate-700 dark:text-slate-300">₱{grandTotal.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</td><td /></tr>
              {totalEwt > 0 && <tr className="bg-slate-50 dark:bg-slate-800"><td colSpan={8} className="px-2 py-1.5 text-right text-xs text-amber-700 dark:text-amber-400">Less: EWT (Withheld)</td><td className="px-2 py-1.5 text-right font-mono text-xs text-amber-700 dark:text-amber-400">({totalEwt.toLocaleString('en-PH', { minimumFractionDigits: 2 })})</td><td /></tr>}
              <tr className="border-t border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-800"><td colSpan={8} className="px-2 py-2 text-right text-xs font-semibold text-slate-700 dark:text-slate-300">Net Payable</td><td className="px-2 py-2 text-right font-mono text-sm font-bold text-slate-900 dark:text-slate-100">₱{netPayable.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</td><td /></tr>
            </tfoot>
          </table>
        </div>
