...
const COLUMNS: ColDef<CMRow>[] = [
  { key: 'memo_no',       header: 'Memo No.',   render: r => <Link href={`/dashboard/ap/credit-memos/${r.id}`} className="font-mono text-xs text-brand-700 hover:underline dark:text-brand-400">{r.memo_no}</Link>, exportValue: r => r.memo_no },
  { key: 'memo_date',     header: 'Date',        render: r => <span className="text-xs text-slate-600 dark:text-slate-400">{formatDate(r.memo_date)}</span>, exportValue: r => formatDate(r.memo_date) },
...
export default function BillCreditMemosPage() {
  ...
  useEffect(() => {
    ...
    const q = status === 'all'
      ? `/ap/credit-memos?company_id=${cid}&limit=500`
      : `/ap/credit-memos?company_id=${cid}&status=${status}&limit=500`;
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Bill Credit Memos</h1>
          <p className="text-sm text-slate-600 dark:text-slate-400">Supplier credit notes reducing AP balances.</p>
        </div>
        <Link href="/dashboard/ap/credit-memos/new" className="rounded bg-brand-600 px-4 py-2 text-sm font-medium text-white hover:bg-brand-700">
          + New Credit Memo
        </Link>
      </div>
      ...
      <DataTable id="ap-credit-memos" columns={COLUMNS} rows={paged} exportRows={rows} loading={loading} filename="ap-credit-memos">
