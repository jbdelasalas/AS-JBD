...
export default function SalesInvoicesPage() {
  ...
  useEffect(() => {
    ...
    setPage(1);
    const q = status === 'all'
      ? `/ar/invoices?company_id=${companyId}&limit=500`
      : `/ar/invoices?company_id=${companyId}&status=${status}&limit=500`;
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Sales Invoices</h1>
          <p className="text-sm text-slate-600 dark:text-slate-400">BIR-compliant sales invoices with automatic GL posting.</p>
        </div>
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={7} className="px-3 py-8 text-center text-xs text-slate-500 dark:text-slate-400">No invoices found.</td></tr>
            ) : paged.map((r) => (
              <tr key={r.id} className="border-b border-slate-100 dark:border-slate-700 last:border-0 hover:bg-slate-50 dark:bg-slate-800">
                ...
                <td className="px-3 py-2 text-xs text-slate-600 dark:text-slate-400">{formatDate(r.invoice_date)}</td>
                <td className="px-3 py-2 text-xs">
                  <span className={r.status === 'overdue' ? 'text-red-600 font-medium' : 'text-slate-600 dark:text-slate-400'}>
