  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Collections</h1>
          <p className="text-sm text-slate-600 dark:text-slate-400">Customer payments and official receipts.</p>
        </div>
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={7} className="px-3 py-8 text-center text-xs text-slate-500 dark:text-slate-400">No collections found.</td></tr>
            ) : paged.map((r) => (
              <tr key={r.id} className="border-b border-slate-100 dark:border-slate-700 last:border-0 hover:bg-slate-50 dark:bg-slate-800">
                ...
                <td className="px-3 py-2 text-xs text-slate-600 dark:text-slate-400">{METHOD_LABELS[r.payment_method] ?? r.payment_method}</td>
