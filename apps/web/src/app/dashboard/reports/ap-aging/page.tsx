...
export default function ApAgingPage() {
  ...
  async function run() {
    ...
    try {
      ...
      const res = await api.get(`/api/v1/reports/ap-aging?${qs}`) as AgingResponse;
      setData(res);
  ...
  const filtered = (data?.rows ?? []).filter((r) =>
    !search || r.supplier_name.toLowerCase().includes(search.toLowerCase()) || r.supplier_tin?.includes(search)
  );
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-slate-900 dark:text-slate-100">AP Aging</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">Outstanding bills by age bucket</p>
        </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {!loading && filtered.length === 0 && (
              <tr><td colSpan={8} className="px-3 py-8 text-center text-slate-400">No outstanding AP as of this date.</td></tr>
            )}
            {filtered.map((r) => (
              <tr key={r.supplier_id} className="border-t border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                <td className="px-3 py-2 text-slate-800 dark:text-slate-200 font-medium">{r.supplier_name}</td>
                <td className="px-3 py-2 font-mono text-xs text-slate-500">{r.supplier_tin || '—'}</td>
      ...
      {data && (
        <div className="mt-2 text-right text-xs text-slate-400">
          {filtered.length} suppliers · computed in {data.duration_ms}ms
        </div>
      )}
    </div>
  );
