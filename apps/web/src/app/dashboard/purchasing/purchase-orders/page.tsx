...
export default function PurchaseOrdersPage() {
  ...
  useEffect(() => {
    ...
    const q = status === 'all'
      ...
      : `/purchasing/purchase-orders?company_id=${companyId}&status=${status}&limit=500`;
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={6} className="px-3 py-8 text-center text-xs text-slate-500">No purchase orders found.</td></tr>
            ) : paged.map((r) => (
              <tr key={r.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-800">
                <td className="px-3 py-2">
                  <Link href={`/dashboard/purchasing/purchase-orders/${r.id}`} className="font-mono text-xs text-brand-700 hover:underline dark:text-brand-400">
                    {r.po_no}
                  </Link>
                </td>
                <td className="px-3 py-2 text-xs text-slate-600 dark:text-slate-400">{formatDate(r.po_date)}</td>
                <td className="px-3 py-2 text-xs text-slate-600 dark:text-slate-400">
                  {r.expected_date ? formatDate(r.expected_date) : '—'}
                </td>
                <td className="px-3 py-2">
