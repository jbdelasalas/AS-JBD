...
const EMPTY_LINE: Line = { line_type: 'item', item_id: '', gl_account_id: '', description: '', quantity: 1, unit_price: 0, vat_rate: 12, grow_reference_id: '' };
...
export default function NewPurchaseOrderPage() {
  ...
  const [form, setForm] = useState({ supplier_id: '', po_date: new Date().toISOString().split('T')[0], expected_date: '', reference: '' });
  ...
  function handleTagChange(field: keyof TaggingValues, val: string) {
    ...
    if (field === 'grow_reference_id') {
      setLines(prev => prev.map(l => ({ ...l, grow_reference_id: val })));
    }
  }
  ...
  function updateLine(idx: number, field: keyof Line, val: string | number) {
    setLines(prev => {
      const next = [...prev];
      const line = { ...next[idx] };
      if (field === 'line_type') { line.line_type = val as 'item' | 'gl'; if (val === 'gl') line.item_id = ''; if (val === 'item') line.gl_account_id = ''; }
      else if (field === 'item_id' && typeof val === 'string') { const item = items.find(i => i.id === val); line.item_id = val; if (item) { line.description = item.name; line.unit_price = item.selling_price; } }
      else (line as Record<string, unknown>)[field] = val;
      next[idx] = line; return next;
    });
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const po = await api.post<{ id: string }>('/purchasing/purchase-orders', {
        company_id: cid, ...form, ...tags,
        expected_date: form.expected_date || undefined, reference: form.reference || undefined,
              ...
              {lines.map((l, idx) => (
                <tr key={idx} className="border-b border-slate-100 dark:border-slate-700">
                  <td className="px-2 py-1">
                    <select value={l.line_type} onChange={e => updateLine(idx, 'line_type', e.target.value)} className="w-full rounded border border-slate-300 px-1 py-1 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100">
                      <option value="item">Item</option><option value="gl">GL Account</option>
                    </select>
                  </td>
                  <td className="px-2 py-1">
                    {l.line_type === 'item' ? (
                      <select value={l.item_id} onChange={e => updateLine(idx, 'item_id', e.target.value)} className="w-full rounded border border-slate-300 px-1 py-1 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100">
                        <option value="">Select…</option>{items.map(i => <option key={i.id} value={i.id}>{i.sku} — {i.name}</option>)}
                      </select>
                    ) : (
                      <select value={l.gl_account_id} onChange={e => updateLine(idx, 'gl_account_id', e.target.value)} className="w-full rounded border border-slate-300 px-1 py-1 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100">
                        <option value="">Select account…</option>{accounts.map(a => <option key={a.id} value={a.id}>{a.code} {a.name}</option>)}
                      </select>
                    )}
                  </td>
                  ...
                  <td className="px-2 py-1"><input type="number" min={0.0001} step="any" value={l.quantity} onChange={e => updateLine(idx, 'quantity', parseFloat(e.target.value) || 0)} className="w-full rounded border border-slate-300 px-1 py-1 text-right dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100" /></td>
                  <td className="px-2 py-1"><input type="number" min={0} step="any" value={l.unit_price} onChange={e => updateLine(idx, 'unit_price', parseFloat(e.target.value) || 0)} className="w-full rounded border border-slate-300 px-1 py-1 text-right dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100" /></td>
                  <td className="px-2 py-1"><input type="number" min={0} step="any" value={l.vat_rate} onChange={e => updateLine(idx, 'vat_rate', parseFloat(e.target.value) || 0)} className="w-full rounded border border-slate-300 px-1 py-1 text-right dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100" /></td>
