...
export default function MasterDataSuppliersPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setPage(1);
    const q = `/ap/suppliers?company_id=${companyId}&limit=500${search ? `&search=${encodeURIComponent(search)}` : ''}`;
    ...
    <div>
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">Supplier master records, TIN, payment terms, and EWT rates.</p>
        </div>
            ...
            {loading ? (
              ...
              <tr key={r.id} className="border-b border-slate-100 dark:border-slate-700 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800">
                ...
                <td className="px-3 py-2">
                  <Link href={`/dashboard/purchasing/suppliers/${r.id}`} className="font-medium text-brand-700 hover:underline dark:text-brand-400">{r.name}</Link>
                ...
                <td className="px-3 py-2 text-right font-mono text-xs">
                  <span className={r.open_ap_balance > 0 ? 'font-medium text-amber-700 dark:text-amber-400' : 'text-slate-500 dark:text-slate-400'}>
