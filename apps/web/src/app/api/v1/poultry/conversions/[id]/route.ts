  ...
  try {
    const [hdr] = await query(
      `SELECT c.*, i.name AS source_item_name, i.sku AS source_sku,
         ...
         LEFT JOIN tally_sheets t ON t.id = c.tally_sheet_id WHERE c.id = $1`, [params.id]);
    if (!hdr) return err('Not found', 404);
    const outputs = await query(
      `SELECT o.*, i.name AS item_name, i.sku FROM conversion_outputs o JOIN items i ON i.id = o.output_item_id
