...
export async function POST(
  ...
  if (po.status !== 'draft') return err(`PO is ${po.status} — only draft POs can be submitted`, 400);

  const updated = await query(
    `UPDATE purchase_orders SET status = 'pending_approval', updated_at = now() WHERE id = $1 RETURNING *`,
    [params.id],
  );
  ...
  await query(
    `INSERT INTO audit_log (user_id, company_id, action, entity_type, entity_id)
     VALUES ($1, $2, $3, $4, $5)`,
    [auth.userId, po.company_id, 'submit', 'purchase_order', params.id],
  ).catch(() => {});
