...
export async function POST(
  ...
  try {
    ...
    if (!['approved', 'partially_delivered', 'fully_delivered'].includes(so.status as string)) {
      ...
      return err(`Cannot close: order is ${so.status}`, 400);
    }

    await client.query(`UPDATE sales_orders SET status = 'closed' WHERE id = $1`, [id]);
    await client.query(
    ...
    await client.query(
      ...
      [auth.userId, so.company_id, 'close', 'sales_order', id],
    ).catch(() => {/* non-fatal */});
